// lib/zmanim.ts
// Daily halachic zmanim (prayer times) via @hebcal/core
// Local calculation — no API latency.
//
// ZIP→lat/lng/timezone lookup now goes through lib/locations.ts,
// which covers every US ZIP (via `zipcodes` package) with the
// correct IANA timezone (via `tz-lookup`). No more silent NY
// fallback for out-of-area users.

import { GeoLocation, Zmanim, HDate, OmerEvent } from '@hebcal/core'
import { lookupZip, LocationInfo } from './locations'

// ============================================
// Format time as "7:23 AM" in the location's local timezone
// ============================================
function fmtTime(date: Date | null, tzid: string): string {
  if (!date) return '--:--'
  return date.toLocaleTimeString('en-US', {
    hour: 'numeric',
    minute: '2-digit',
    hour12: true,
    timeZone: tzid,
  })
}

// ============================================
// Calculate daily zmanim for a ZIP code
// ============================================
export interface DailyZmanim {
  date: Date
  location: string
  tzid: string
  alotHaShachar: Date | null
  sunrise: Date | null
  sofZmanShma: Date | null
  sofZmanTfilla: Date | null
  chatzot: Date | null
  minchaGedola: Date | null
  plagHaMincha: Date | null
  sunset: Date | null
  tzeit: Date | null
}

/**
 * Returns zmanim for a resolved location. Pass the result of
 * lookupZip(). We intentionally require a resolved location (not
 * a raw ZIP string) so callers explicitly handle the "unknown ZIP"
 * case and don't silently fall back to NYC.
 */
export function getDailyZmanim(loc: LocationInfo): DailyZmanim {
  const gloc = new GeoLocation(loc.city, loc.lat, loc.lng, 0, loc.tzid)
  const today = new Date()
  const z = new Zmanim(gloc, today, false)

  return {
    date: today,
    location: `${loc.city}, ${loc.state}`,
    tzid: loc.tzid,
    alotHaShachar: z.alotHaShachar(),
    sunrise: z.sunrise(),
    sofZmanShma: z.sofZmanShma(),
    sofZmanTfilla: z.sofZmanTfilla(),
    chatzot: z.chatzot(),
    minchaGedola: z.minchaGedola(),
    plagHaMincha: z.plagHaMincha(),
    sunset: z.sunset(),
    tzeit: z.tzeit(8.5),
  }
}

// ============================================
// Format zmanim for SMS (~2 segments)
// ============================================
// ============================================
// Hebrew date + Sefirat HaOmer helpers
// ============================================

/**
 * Compute the day of Sefirat HaOmer for a given Hebrew date.
 * Returns 1-49 if in Omer period (16 Nisan through 5 Sivan), else null.
 */
function getOmerDay(hd: HDate): number | null {
  const month = hd.getMonth()        // 1=Nisan, 2=Iyar, 3=Sivan
  const day = hd.getDate()

  // Nisan 16 through Nisan 30: days 1-15 of Omer
  if (month === 1 && day >= 16 && day <= 30) return day - 15

  // Iyar 1 through Iyar 29: days 16-44 of Omer
  if (month === 2) return day + 15

  // Sivan 1 through Sivan 5: days 45-49 of Omer
  if (month === 3 && day >= 1 && day <= 5) return day + 44

  return null
}

/**
 * Format Omer day count in English and Hebrew
 * e.g. day 33 → "Day 33 — ל״ג לעומר (4 weeks, 5 days)"
 */
function formatOmer(day: number): string {
  const weeks = Math.floor(day / 7)
  const extraDays = day % 7

  let weekPart = ''
  if (weeks > 0) {
    weekPart = weeks === 1 ? '1 week' : `${weeks} weeks`
    if (extraDays > 0) {
      weekPart += extraDays === 1 ? ', 1 day' : `, ${extraDays} days`
    }
  }

  // Special named days
  const special: Record<number, string> = {
    33: 'Lag BaOmer',
  }
  const nameParts: string[] = [`Day ${day} of the Omer`]
  if (special[day]) nameParts.push(`(${special[day]})`)
  if (weekPart) nameParts.push(`— ${weekPart}`)

  return nameParts.join(' ')
}

export function formatZmanimForSMS(zipCode?: string | null): string {
  const loc = lookupZip(zipCode)
  if (!loc) {
    return `🕐 Zmanim

Couldn't find that ZIP code.
Please send your 5-digit US ZIP code.
Example: zmanim 11213

If you're outside the US, email
contact@connect2kehilla.com and we'll
add your area.`
  }

  const z = getDailyZmanim(loc)
  const dayName = z.date.toLocaleDateString('en-US', {
    weekday: 'short',
    month: 'short',
    day: 'numeric',
    timeZone: loc.tzid,
  })

  // Hebrew date — render in Hebrew script
  const hd = new HDate(z.date)
  const hebrewDate = hd.render('he')   // e.g. "י׳ ניסן תשפ״ו"

  // Sefirat HaOmer
  const omerDay = getOmerDay(hd)
  const omerLine = omerDay ? `\n🌾 ${formatOmer(omerDay)}\n` : ''

  return `🕐 Zmanim — ${z.location}
${dayName}
${hebrewDate}${omerLine}
עלות Dawn: ${fmtTime(z.alotHaShachar, loc.tzid)}
הנץ Sunrise: ${fmtTime(z.sunrise, loc.tzid)}
סוף ק״ש Shma: ${fmtTime(z.sofZmanShma, loc.tzid)}
סוף תפילה Tfilla: ${fmtTime(z.sofZmanTfilla, loc.tzid)}
חצות Midday: ${fmtTime(z.chatzot, loc.tzid)}
מנחה גדולה Mincha: ${fmtTime(z.minchaGedola, loc.tzid)}
פלג Plag: ${fmtTime(z.plagHaMincha, loc.tzid)}
שקיעה Sunset: ${fmtTime(z.sunset, loc.tzid)}
צאת Nightfall: ${fmtTime(z.tzeit, loc.tzid)}

Reply MENU for options.`
}
